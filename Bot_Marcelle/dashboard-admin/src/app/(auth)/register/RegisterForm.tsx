...
export function RegisterForm({ invite, inviteToken }: Props) {
  const [state, action, pending] = useActionState<State, FormData>(registerAction, null)
  const hasInvite = invite !== null

  // Se veio com token mas o convite é inválido/expirado, avisa
  const invalidInvite = inviteToken && !invite
  ...
  return (
    <div className="w-full max-w-sm">
      <div className="flex flex-col items-center gap-2 mb-8">
        ...
        <h1 className="text-2xl font-semibold text-zinc-900 dark:text-white">Criar conta</h1>
        <p className="text-sm text-zinc-500 dark:text-zinc-400">Acesso ao painel Assinatura Admin</p>
      </div>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 flex flex-col gap-4 shadow-sm">
        ...
        {hasInvite ? (
          <div className="flex items-start gap-2.5 px-3 py-2.5 rounded-lg bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-100 dark:border-emerald-900/60">
            ...
            <p className="text-xs text-emerald-700 dark:text-emerald-300 leading-relaxed">
              ...
              para <span className="font-semibold">{invite.email}</span>.
            </p>
          </div>
        ) : invalidInvite ? (
          <div className="flex items-start gap-2.5 px-3 py-2.5 rounded-lg bg-red-50 dark:bg-red-950/40 border border-red-100 dark:border-red-900/60">
            <Info className="w-4 h-4 text-red-500 dark:text-red-400 mt-0.5 shrink-0" />
            <p className="text-xs text-red-700 dark:text-red-300 leading-relaxed">
              Convite inválido ou expirado. Peça um novo link ao administrador.
            </p>
          </div>
        ) : (
          <div className="flex items-start gap-2.5 px-3 py-2.5 rounded-lg bg-violet-50 dark:bg-violet-950/40 border border-violet-100 dark:border-violet-900/60">
            ...
            <p className="text-xs text-violet-700 dark:text-violet-300 leading-relaxed">
              ...
              são aceitos. Para outros domínios, peça um convite ao administrador.
            </p>
          </div>
        )}
        ...
        {!hasInvite && (
          <>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-zinc-400 dark:text-zinc-500">ou com e-mail</span>
              <div className="flex-1 h-px bg-zinc-100 dark:bg-zinc-800" />
            </div>
          </>
        )}
        ...
        <form action={action} className="flex flex-col gap-4">
          {hasInvite && <input type="hidden" name="invite_token" value={inviteToken} />}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Input
              name="email"
              type="email"
              placeholder={hasInvite ? invite.email : 'voce@assinaturamarcapropria.com.br'}
              defaultValue={hasInvite ? invite.email : ''}
              readOnly={hasInvite}
              required
              autoComplete="email"
              className={hasInvite ? 'bg-zinc-50 dark:bg-zinc-950 cursor-not-allowed' : ''}
            />
          </div>

          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Senha</label>
            <PasswordInput name="password" placeholder="Mínimo 8 caracteres" required autoComplete="new-password" />
          </div>

          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Confirmar senha</label>
            <PasswordInput name="confirm" placeholder="Repita a senha" required autoComplete="new-password" />
          </div>

          <Button type="submit" loading={pending} className="w-full mt-2" disabled={!!invalidInvite}>
            Criar conta
          </Button>
        </form>
